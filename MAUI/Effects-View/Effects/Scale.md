...
The example below shrinks the `Effects View` to 85% of its current size when the user long-presses it. The other trigger properties are set to `None` so that only the `Scale` effect fires.
...
- [Rotation](https://help.syncfusion.com/maui/effects-view/effects/rotation) 
