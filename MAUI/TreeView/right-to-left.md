...
The `SfTreeView` control supports displaying its content in a right-to-left (RTL) layout for languages such as Arabic, Hebrew, and Urdu. To enable RTL, set the [FlowDirection](https://learn.microsoft.com/dotnet/api/microsoft.maui.flowdirection) property to `RightToLeft`. The `FlowDirection` can be set directly on the `SfTreeView` or inherited from its parent layout. By default, the `FlowDirection` is set to `LeftToRight`.
