...
Use the `Indeterminate` state to display in-progress work, such as a partially completed task. Enable it by setting [AllowIndeterminateState](https://help.syncfusion.com/cr/maui/Syncfusion.Maui.Buttons.SfSwitch.html#Syncfusion_Maui_Buttons_SfSwitch_AllowIndeterminateState) to `true` and `IsOn` to `null`.
