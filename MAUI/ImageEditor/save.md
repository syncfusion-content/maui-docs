...
The `SavePickerOpening` event occurs while the save picker is opening on the save icon click.
Cancel: We can restrict the save opening by setting the Cancel argument to true. If save picker is disabled, the image will be saved in the default location.
