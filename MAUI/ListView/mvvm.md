...
- .NET MAUI workload installed (Visual Studio 2022 17.8+ or .NET 8+ SDK).
- Syncfusion.Maui.ListView NuGet package (version 25.1.35 or later recommended).
- `Syncfusion.Maui.ListView` handler registered in `MauiProgram.cs` with `.ConfigureSyncfusionListView()`.
- The samples below assume the `Syncfusion.Maui.ListView` namespace is imported in XAML:
...
<ContentPage xmlns="http://schemas.microsoft.com/dotnet/2021/maui"
             ...
             xmlns:converter="clr-namespace:ListViewSample.Converters"
             xmlns:behavior="clr-namespace:ListViewSample.Behaviors"
             xmlns:syncfusion="clr-namespace:Syncfusion.Maui.ListView;assembly=Syncfusion.Maui.ListView"
             x:Class="ListViewSample.MainPage">
    <ContentPage.Resources>
        <ResourceDictionary>
            <converter:EventArgsConverter x:Key="EventArgs"/>
        </ResourceDictionary>
    </ContentPage.Resources>
...
The samples that use `Behaviors` and `Converter` rely on a small helper that converts any .NET event into an `ICommand`. Add the following `EventToCommandBehavior` and `EventArgsConverter` to your project (typically under `Behaviors/` and `Converters/` folders). Make sure to import them in XAML, e.g.:
...
public class EventToCommandBehavior : Behavior<SfListView>
{
    ...
    private SfListView associatedObject;
    ...
    protected override void OnAttachedTo(SfListView bindable)
    {
        ...
        eventHandler = Delegate.CreateDelegate(eventInfo.EventHandlerType, this,
    ...
    private void OnEventRaised(object sender, object eventArgs)
    {
        if (Command == null) return;
        var parameter = Converter?.Convert(eventArgs, typeof(object), null, CultureInfo.InvariantCulture) ?? eventArgs;
...
N> `MainPage` is deprecated in .NET MAUI 9+. Prefer `Application.Current.Windows[0].Page` or inject the active `Page` into the ViewModel.

## Event-args reference

The command samples below pass the following event-args types as the command parameter:

| Event-args type | Property | Description |
|---|---|---|
| `ItemTappedEventArgs` | `DataItem` | The underlying data object for the tapped item. |
| `ItemRightTappedEventArgs` | `DataItem` | The underlying data object for the right-tapped item. |
| `ItemLongPressEventArgs` | `DataItem` | The underlying data object for the long-pressed item. |
| `ItemLongPressEventArgs` | `ItemType` | The `ItemType` (for example, `Item`, `Header`, `Footer`, or `LoadMore`). |
| `ItemLongPressEventArgs` | `Position` | The tap position relative to the item. |
| `ItemSelectionChangedEventArgs` | `AddedItems` | The list of items that became selected. |
| `ItemSelectionChangedEventArgs` | `RemovedItems` | The list of items that became deselected. |
| `ItemSelectionChangingEventArgs` | `AddedItems` | The list of items about to become selected. |
| `ItemSelectionChangingEventArgs` | `RemovedItems` | The list of items about to become deselected. |
| `ItemSelectionChangingEventArgs` | `Cancel` | Set to `true` to cancel the selection. |
