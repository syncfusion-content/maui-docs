...
description: Learn about Customization support with Syncfusion .NET MAUI Step ProgressBar (SfStepProgressBar).
platform: maui
...
Customize the progress animation duration and background by using the [ProgressAnimationDuration](), [ProgressBarBackground]() properties in `SfStepProgressBar`. Please find the following code example that explains how to use `ProgressAnimationDuration` and `ProgressBarBackground` in the step progress bar.
