...
Suggestions can be shown in a item by creating a `AssistSuggestion` instance and setting it to the desired item's `Suggestions` property.
...
{% highlight c# tabtitle="ViewModel.cs" hl_lines="13 14" %}
 ...
 public class SuggestionsViewModel : INotifyPropertyChanged
 {
    ...
    public SuggestionsViewModel()
    {
        ...
        suggestions = new ObservableCollection<ISuggestion>();
        suggestions.Add(new AssistSuggestion() { Text = "Get started with .NET MAUI", Image Source = "learn_more.png" });
...
{% highlight c# tabtitle="ViewModel.cs" hl_lines="13" %}
    ...
    public SuggestionsViewModel()
    {
        ...
        assistSuggestions.Orientation = SuggestionsOrientation.Vertical;
     }
...
By default, when tapping or clicking the suggestion item, it will be sent as a request item immediately. So, if you wish to prevent this behavior and show the suggestions list in editor view, you can set the `SuggestionItemSelectedEventArgs.CancelRequest` to `true` within the `SuggestionItemSelected` event handler or the `SuggestionItemSelectedCommand` command's execution.
