...
## AutoScrollBehavior in SfAIAssistView
The `SfAIAssistView` control provides support for customizing the automatic scrolling behavior using the [AutoScrollBehavior]() property. 

This feature allows developers to control whether the view scrolls to the latest AI response or remains anchored to the user's most recent request.

The `AutoScrollBehavior` property supports the following values:

 * [ScrollToLastResponse]()
Automatically scrolls to the latest AI response.This is the default behavior.
 * [ScrollToLastRequest]()
Automatically scrolls to the most recent user request (prompt), ensuring the user’s input remains visible even when the AI response is long or streaming.
