...
description: Learn about migrating from the Syncfusion® Xamarin Range Slider to the Syncfusion® .NET MAUI Range Slider (SfRangeSlider) control.
platform: MAUI
...
To migrate from the [Xamarin Range Slider](https://www.syncfusion.com/xamarin-ui-controls/xamarin-range-slider) control to .NET MAUI, use the following two controls.

1. SfDateTimeRangeSlider – For date-time scale.
2. SfRangeSlider – For numeric scale.

In .NET MAUI SfRangeSlider, there is no `ShowRange` API. Both the .NET MAUI SfDateTimeRangeSlider and SfRangeSlider controls display two thumbs by default. If you want to use a single thumb, see the [Migration from Xamarin.Forms SfRangeSlider to .NET MAUI SfSlider](https://help.syncfusion.com/maui/slider/migration) documentation.
...
<td>Gets or sets the brush for the inactive ticks in the SfRangeSlider.</td></tr>
<tr>
...
<td>Gets or sets the brush for the active ticks in the SfRangeSlider.</td></tr>
</table> 
