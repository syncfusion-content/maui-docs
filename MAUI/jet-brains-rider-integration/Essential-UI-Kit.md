...
N> Before using the **Essential® UI Kit for .NET MAUI - Syncfusion®**, ensure that the extension is installed in JetBrains Rider. Check the Plugins list in **Settings/Preferences > Plugins**. If it is not installed, follow the steps in the [download and installation](download-and-installation) guide to install it.
